...
class ConfNet :
    def __init__(self, stride, num_of_kernels, kernel_size, num_of_nodes_in_last_layer, learning_rate, image_size):
    ...
    def initialize_weights(self):
        ...
        num_of_image_fragments = round(((self.image_size - self.kernel_size)/self.stride + 1) * \
                                 ((self.image_size - self.kernel_size)/self.stride + 1))
        num_of_flattened_conv_outputs = self.num_of_kernels * num_of_image_fragments
    ...
    def extract_and_transform_image(self, image):
        fragments_of_image = []
        image_s = round((self.image_size - self.kernel_size)/self.stride + 1)

        for i in range(image_s):
            for j in range(image_s):
                fragment = image[i*self.stride: i*self.stride+self.kernel_size, j*self.stride: j*self.stride+self.kernel_size]
    ...
    def perform_convolution(self, image):
        ...
        conv_output = self.activate_relu(np.dot(self.fragments_of_image, self.conv_kernel_reshaped))
        self.conv_output = conv_output.flatten()

    def perform_fully_connected_l(self):
        self.fc_output = np.dot(self.fc_weights, self.conv_output)
    ...
    def back_propagation(self, expected_res):
        ...
        conv_delta = conv_delta.reshape(len(self.fragments_of_image), self.num_of_kernels)

        proper_deltas = [np.dot(conv_delta.T, self.fragments_of_image),
                         fc_delta.reshape(-1, 1) * np.array(self.conv_output).T]
    ...
    def calculate_conv_delta(self, output_layer_delta):
    ...
    def calculate_delta_after_derivative(self, layer_delta):
        after_derivative = np.array([1 if value > 0 else 0 for value in self.conv_output])
...
def main():
    np.random.seed(1)
    network = ConfNet(1, 16, 3, 10, 0.1, 28)
