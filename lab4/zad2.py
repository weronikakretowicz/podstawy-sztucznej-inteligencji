...
class NeuralNetwork:
    ...
    def count_deltas(self, outputs_of_all_series, expected_results, weights, outputs_of_layers, dropout_masks):
        ...
        # if 1 in self.activation_functions:
    ...
    def fit(self, expected_results, features):
        ...
        for epoc in range(0, 1000):
            ...
            while i + BATCH_SIZE <= NUM_OF_SERIES:
                ...
                curr_inputs = features[:, start:end]
                total_outputs = np.array(self.predict(curr_inputs, weights, dropout_masks[start:end]))
                outputs_max = self.find_index_with_max_value(total_outputs)
                expected_outputs_max = self.find_index_with_max_value(expected_results[:, start:end])
                ...
                for j in range(0, num_of_layers):
                    curr_inputs = (self.calculate_layer_output(weights[j], curr_inputs, j + 1, dropout_masks[start:end]))

                    all_outputs.append(curr_inputs)

                proper_layers_deltas = self.count_deltas(curr_inputs, expected_results[:, start:end],
                                                         weights, all_outputs, dropout_masks[start:end])
